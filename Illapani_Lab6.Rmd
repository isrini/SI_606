...
The percentages reported are based on sample statistics from represenative county.  
...
The assumption is reasonable but we must also remember that the sample popolations are not proportinal to the
size of the country, this could yield results that may not be fully representative of say a large nation like
China or India.
...
atheist
total_resp <- nrow(us12)
total_resp

atheist/total_resp
```

The proportion of atheist response comes to 5% and this number matches with the one in Table 6 for USA.  
...
The conditions are that the sample is less than 10% of the total population, which is applicable
to the US with 1002 people surveyed. The samples should be independent and in this scenario, they
are independent responses from individuals.
...
As p increases from 0 to .5, me also increases.  
But as p increases from -5 to 1.0, me decreases and goes down to 0.
...
The samplng distribution is centered at 0.10 of the population proportion.
The distribution is shaped normal.
...
The change in p (increase) moves the curve or bars towards the center of the distribution.
The change in n (increase) makes the spread smaller.  
...
There is no significant different in the margin of error between Australia and Ecuador. 
It is ok to procees and make an inference here.
...
The confidence intervals for 2005 and 2012 overlap, hence we have to go with the
null hypothesis that there is no significant difference in atheism rates between
...
inference(usa2005$response, est = "proportion", type = "ci", method = "theoretical", 
          success = "atheist")

usa2012 <- subset(atheism, nationality == "United States" & year == "2012")
inference(usa2012$response, est = "proportion", type = "ci", method = "theoretical", 
          success = "atheist")
```

The confidence intervals for US for years 2005 and 2012 do not overlap, hence we have to reject the
null hypothesis and infere that there is a difference in atheism rates between the years 2005 and 2012.
...
Type I error is the false rejection of the null hypothesis. One would expect to interpret correctly
a change 95% of the time, and not so 5% of the time due to chance.
...
The number of people we need to sample to ensure one is within the guidelines is 9,604  

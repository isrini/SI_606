...
```
    ...
    Using the scatter plot the relationship does appear to be linear. Yes, I would be
    comfortable using this model to predict the number of runs.
...
2.  Looking at your plot from the previous exercise, describe the relationship 
    ...
    The variables appear to have a positive linear correlation. As at_bats increase,
    runs also increase.
...
3.  Using `plot_ss`, choose a line that does a good job of minimizing the sum of
    ...
    Smallest sum of squares is 142182.4
    Largest is 207215.9
...
```   
    ...
    We see a positive relationship, for every 1 homerun we would see an increase in 1.8345 runs.
...
5.  If a team manager saw the least squares regression line and not the actual 
    ...
    About 727 runs predicted for a team with 5578 at-bats.
    The estimate is close to the regression line and the residual value should be minimum or very low.
...
6.  Is there any apparent pattern in the residuals plot? What does this indicate
    ...
    There is no apparant pattern in the residuals plot. It indicates that
    there is a minimal linearity as there is no standard pattern.
...
7.  Based on the histogram and the normal probability plot, does the nearly 
   ...
   The distribution is not normal but it is somewhat close to a normal shape.
...
8.  Based on the plot in (1), does the constant variability condition appear to 
    ...
    Yes, the constant variability condition seems to be satisfied here.
...
```
    ...
    When 'hits' are used as another predictor of runs, the liner model shows a pattern but it
    is not truly predictable of runs as we do not see a consistent linear pattern.
...
```
    ...
    The R2 for 'runs' is higher than the one for 'at_bats'. As one can see from the m1 and m2
    summary results. Hence, the variable I selected, 'hits' is better than 'at_bats'.
...
```   
    ...
    'bat_avg' best predicts runs among the five traditional variables.
    As you can see from the plot and the Summary results of m3.
...
```  
    ...
    'new_obs' best predicts runs among the three new variables.
    As you can see from the plot and the Summary results of m4.  
...
-   Check the model diagnostics for the regression model with the variable you 
    ...
    Based on the summary resuts for m4, the new_obs is the best predictor as
    we see a very low or negligible p-value and a large R2 value.

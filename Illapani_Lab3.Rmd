...
```{r height-hist, eval=TRUE}   
hist(mdims$hgt)
hist(fdims$hgt)
```


  - The height distribution for men has a more normal distrubtion compared to the one for women.
...
2.  Based on the this plot, does it appear that the data follow a nearly normal 
    ...
    - The plot above shows that the height data for females is close to a normal distribution.
...
```
  ...
  - Majority of the points fall on the line, but there are some at the bottom and at the top of the line that do not fall on the line.
  This plot is similar to the probability plot but the data points here are tightly aligned to the line compared to the probability plot.
...
4.  Does the normal probability plot for `fdims$hgt` look similar to the plots 
    ...
    - The normal probability plot looks similar to the simulated data plot. They both 
    have outlier points, but it looks like the simulated data points are much closer to the line.
...
- The weight of females are close to normal in the middle area. Near the upper endd, some deviations are observed.
...
1. Calculate the probability that a randomly selected female is taller than 155cm?

```{r, eval=TRUE}
p155_t <- sum(fdims$hgt > 155)/length(fdims$hgt)
p155_t
```

```{r, eval=TRUE}
p155_e <- 1 - pnorm(q=155, mean = fhgtmean, sd = fhgtsd)
p155_e
```


2. Calculate the probability that a randomly selected female weighs more than 55 kilos?


```{r, eval=TRUE}
p55_t <- sum(fdims$wgt > 55) / length(fdims$wgt)
p55_t
```
...
p55_e <- pnorm(q=55, mean = fwgtmean, sd = fwgtsd)
p55_e
```

***
- Weight has closer agreement compared to height between the two probability methods.  
...
-   Note that normal probability plots C and D have a slight stepwise pattern.  
    ...
    - It is due to the data points progression for heights going up and down.
...
- The normal probability curve illustrates a slightly right skewed knee diameter data.  
The histogram confirms the same.

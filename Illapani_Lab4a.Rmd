...
2.  Describe the distribution of this sample. How does it compare to the 
    ...
    As seen in the histogram below, the sample distribution is normal and
    skewed to right a bit. The QQ plot is more or less a normal distribution 
...
3.  Take a second sample, also of size 50, and call it `samp2`.  How does the 
    ...
    The mean of the two samples of size 5o each is almost same. The larger the
    sample the more accurate is the estimate of the population mean.  

```{r, eval=TRUE}
samp100 = sample(area, 100)
samp1000 = sample(area, 1000)
m_samp100 = mean(samp100)
m_samp1000 = mean(samp1000)
df_means = data.frame(Mean1=c(m_samp100), Mean2=c(m_samp1000))
...
4.  How many elements are there in `sample_means50`?  Describe the sampling 
    ...
    means?
     ...
     Sample_means50 has 5000 elements. The sampling distribution has a 
     normal shape, unimodal with a mean of approx 1500. In the QQ plot, 
     the sampling distribution is in alignment to the line. For 50,000 sample means,
     the distribution would more closely match the normal distribution.  
...
5.  To make sure you understand what you've done in this loop, try running a 
    ...
    There are 100 elements and each of them represent the mean of a sample
    taken from the area population.  
...
6.  When the sample size is larger, what happens to the center?  What about the spread?  
    ...
    When the sample size is larger, the center appears to be more prominent. The
    spread would narrow.
...
-   Since you have access to the population, simulate the sampling 
    ...
    The sampling distribution has a normal bell curve. It is unimodal and skewed centrally.
    The mean home price of the population could be somewhere in between $176K and $179k.  
...
-   Change your sample size from 50 to 150, then compute the sampling 
    ...
    The sampling distribution has a normal bell curve. It is unimodal and skewed centrally.
    Compared to the previous one, this shape is slightly more uniform. The mean home price of
    the population could be somewhere in between $178K and $180k.  
...
-   Of the sampling distributions from 2 and 3, which has a smaller spread?  If
    ...
    The sample of 150 has the smaller spread. The standard deviation measures for both reveal that
    the larger sample size the smaller the spread or deviation from the mean.  

```{r, eval=TRUE} 
sd_samp50 <- sd(sample_means50)
sd_samp150 <- sd(sample_means150)

df_Sd <- data.frame(sd_samp50, sd_samp150)
df_Sd
```
